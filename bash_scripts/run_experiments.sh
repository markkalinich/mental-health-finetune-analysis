...
check_experiment_cache() {
    local family=$1 size=$2 version=$3
    run_python_module utilities.cache_checker \
        --model-family "${family}" \
        --model-size "${size}" \
...
run_model_experiment() {
    ...
    cache_percentage=$(check_experiment_cache "${family}" "${size}" "${version}" | tail -1)
