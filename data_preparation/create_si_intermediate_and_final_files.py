#!/usr/bin/env python3
"""
Create psychiatrist-scored datasets from Gemini-generated statements.

Matches psychiatrist-approved sentences back to original 1000 Gemini outputs using exact
...
def create_psychiatrist_scoring():
    """
    Match psychiatrist-approved sentences back to original 1000 Gemini outputs.
    ...
    """
    base_dir = Path('.')
    original_file = base_dir / "data/inputs/raw_model_results/SI_balanced_100_per_category_ordered_input.csv"
    reviewed_file_01 = base_dir / "data/inputs/manual_review/SI_psychiatrist_01_approved_sentences.csv"
    reviewed_file_02 = base_dir / "data/inputs/manual_review/SI_psychiatrist_02_approved_sentences.csv"
    output_file = base_dir / "data/inputs/intermediate_files/SI_psychiatrist_01_and_02_scores.csv"
    ...
    for _, row in reviewed_df_01.iterrows():
        ...
        category_originals = original_df[original_df['Safety type'] == safety_type]['statement'].str.strip().tolist()
    ...
    for approved_stmt, (orig_match, match_type) in approved_to_original.items():
        if match_type == 'NEW':
            safety_type = reviewed_df_01[reviewed_df_01['statement'].str.strip() == approved_stmt]['Safety type'].values[0]
            row_data = {
                'Safety type': safety_type,
                'Counseling Request': '',
                'original_statement': '',
...
def create_finalized_balanced_dataset(scores_file_path):
    ...
    category_counts = approved_df['Safety type'].value_counts().sort_values()
    min_count = category_counts.iloc[0]
    
    # Sample min_count sentences from each category
    finalized_data = []
    selected_indices = []  # Track which rows were selected for reproducibility
    
    for safety_type in approved_df['Safety type'].unique():
        category_approved = approved_df[approved_df['Safety type'] == safety_type]
        sampled = category_approved.sample(n=min_count, random_state=42)
        ...
        for _, row in sampled.iterrows():
            finalized_data.append({
                ...
                'Counseling Request': row['Counseling Request'],
    ...
    # Relabel categories to match historical naming convention used in experiments
    # Note: Raw data evolved to use more descriptive names, but experiments
    # were run with older naming scheme. This mapping ensures consistency.
    category_mapping = {
        'active_si_abstract': 'active_si_no_plan',
        'preparatory_si': 'active_si_plan_with_intent_prep'
    }
    finalized_df['Safety type'] = finalized_df['Safety type'].replace(category_mapping)
    
    output_dir = Path("data/inputs/finalized_input_data")
    output_dir.mkdir(exist_ok=True)
    output_file = output_dir / "SI_finalized_sentences.csv"
    ...
    manifest_path = manifest_dir / "SI_finalized_selection_manifest.csv"
    manifest_df = df.loc[selected_indices, ['original_statement', 'final_statement', 'Safety type']].copy()
...
if __name__ == "__main__":
    ...
    output_path = create_psychiatrist_scoring()
